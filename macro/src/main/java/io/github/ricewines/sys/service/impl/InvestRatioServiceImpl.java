...
//    @Scheduled(cron = "0 29 23 3 2 *")
    public void positionAddAndAdjust() {

        String taskName = "加仓和调仓的投资比例";
        // 初始化客户端
        log.info("开始执行，任务名：{}", taskName);
        ZhipuAiClient client = ZhipuAiClient.builder().ofZHIPU()
                .apiKey(investConfig.getZhiPuAi().getApiKey())
                .build();

        Resource resource = applicationContext.getResource("classpath:投资比例提示词.md");
        String content;
        try {
        ...
        if (response.isSuccess()) {
            ...
            try {
                ...
                String subject = "%s投资比例提示"
                        .formatted(localDate.format(DateTimeFormatter.ofPattern("yy年MM月", Locale.CHINA)));
